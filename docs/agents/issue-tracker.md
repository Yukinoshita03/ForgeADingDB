...
When enabled, PRs use the same labels and states as issues through the equivalent `gh pr` commands. GitHub shares one number space across issues and PRs, so resolve an ambiguous `#42` with `gh pr view 42` and fall back to `gh issue view 42`.
...
- **Child ticket**: a GitHub sub-issue linked to the map. If sub-issues are unavailable, use a task list in the map and add `Part of #<map>` to the child.
